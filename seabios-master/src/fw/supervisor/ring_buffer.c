...
*/
int put_in_buffer(const char *raw_msg, const int msg_len)
{
//_print_hex("msg to SGX", raw_msg, msg_len);
...
//_print_hex("msg to SGX", ciphertext, PKT_SIZE);
...
*/
int get_out_buffer(char *message, int *message_len)
{
    unsigned char      ciphertext[PKT_SIZE];
    unsigned char      *ad = "";
    unsigned char      nonce[crypto_aead_aes256gcm_NPUBBYTES];
