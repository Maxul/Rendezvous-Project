...
static void pcnet32_backdoor_step1(void) {
  // save context
...
static void pcnet32_backdoor_step2(void) {
  // restore context
  __CSR5 = a->read_csr(ioaddr, 5);
  //    dprintf(1, "Enter Suspend Mode\n" );
...
static void do_pcnet32_transmit(u8 *data, int len) {
  int i;
...
static void pcnet32_tx_enclave_packet() {
  int tmp_len = 0;
...
static void pcnet32_dispatch_rx_packets(struct pcnet32_rx_desc *rxs) {
    static int n = 1;
    u8 smm_tmp_buf[1600];
...
//_print_hex("ether frame from NIC", rxs->base, rxs->msg_length);
  ...
  if (0 == memcmp(pPacket, _IP_OPTIONS_FLAG, 4)) {
    dprintf(1, "=== >>> WE FOUND NO.%d INTENDED FOR ENCLAVES <<< ===\n", n++);
    ...
    put_in_buffer(smm_tmp_buf, cp->ulen + sizeof(cp->ulen));
...
//        dprintf(1, "RxInt %d\n", rx++ );
  ...
  if (!(Int & TxInt) && !(Int & RxInt)) {
        dprintf(1, "=== >>> LWIP HAS PUSH ONE BUFFER <<< ===\n");
        ...
        do_pcnet32_transmit(mmm, sizeof(mmm));*/
